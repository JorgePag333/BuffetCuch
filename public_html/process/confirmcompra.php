...
$comprobanteDir="../assets/comprobante/";
$Sumador=0;
$detalles="";
$cant=1;
$hora="";
...
if(mysqli_num_rows($verdata)>=1){
  if(!empty($comprobanteType)){
    if($comprobanteType=="image/jpeg" || $comprobanteType=="image/png"){
      if(($comprobanteSize)<=$comprobanteMaxSize){
  ...
  if(!empty($_SESSION['carro']) || (!empty($_SESSION['carro1'])) ){
    ...
    if (consultasSQL::InsertSQL("venta", "Fecha, NIT,Descuento, TotalPagar, Estado, NumeroDeposito, TipoEnvio, Adjunto", "'".date('d-m-Y')."','$Cedclien','0','$suma','$StatusV','$NumDepo','$tipoenvio','$comprobanteF'")) {
...
foreach ($_SESSION['carro1'] as $code) {
    ...
    if ($hora == "") {
        ...
        foreach ($_SESSION['carro'] as $carro) {
            ...
            consultasSQL::InsertSQL("detalle", "NumPedido, CodigoProd, CantidadProductos, PrecioProd, NombreProd, Detalles, hora", "'$Numpedido', '".$carro['producto']."', '".$carro['cantidad']."', '$Sumador',' $desacri','$detalles','$hora'");
