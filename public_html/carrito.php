    ...
    <section id="container-pedido">
        <div class="container">
            <div class="page-header">
              <h1>CARRITO DE COMPRAS <small class="tittles-pages-logo">Buffet del CUCh Online</small></h1>
            </div>
            <br><br><br>
            <div class="row">
                <div class="col-xs-12">
                    <?php
                     ...
                     $adre="";
                        ...
                        if(!empty($_SESSION['carro1'])){
                            foreach ($_SESSION['carro1'] as $code) {
                                        $consu=ejecutarSQL::consultar("SELECT * FROM adicionales WHERE indi='".$code['detalle']."'");
                                        while ($fila = mysqli_fetch_array($consu, MYSQLI_ASSOC)) {
                                            $Sumador = $Sumador + $fila['precioAd'];
                                            $detalles .= $fila['nombre'] .=", ";
                                        }
                                   
                                        if($hora == ""){
                                            $hora .= $code['horaPed'];
                                        }
                                        if(isset($code['detalleAd']) && $adre==""){
                                            $consu11=ejecutarSQL::consultar("SELECT * FROM adereso WHERE indice='".$code['detalleAd']."'");
                                            while ($fila11 = mysqli_fetch_array($consu11, MYSQLI_ASSOC)) {
                                                $Sumador = $Sumador + $fila11['precioCat'];
                                                $adre .= $fila11['tipo'] .=", ";
                                            }
                                        }
                                    }
                                    $detalles .= $adre;
                        $detalles .= $hora;
                                    if(!empty($_SESSION['carro2']) ||!empty($_SESSION['carro1']) ){
                                        $suma = 0;
                                        $sumaA = 0;
                                        echo '<table class="table table-bordered table-hover"><thead><tr class="bg-success"><th>Nombre</th><th>Detalle</th><th>Precio</th><th>Cantidad</th><th>Subtotal</th><th>Acciones</th></tr></thead>';
                                        ...
                                        echo '<tr class="bg-danger"><td colspan="2">Total</td><td><strong></strong></td><td><strong>$'.number_format($sum,2).'</strong></td></tr></table><div class="ResForm"></div>';
                                        echo '
                                        <p class="text-center">
