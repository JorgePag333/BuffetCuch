        ...
        <div class="container">
            <div class="row">
                <div class="page-header">
                    <h1>DETALLE DE PRODUCTO <small class="tittles-pages-logo">Buffet del CUCh Online</small></h1>
                </div>
                <?php 
                  $CodigoProducto=consultasSQL::clean_string($_GET['CodigoProd']);
                  $codigoSub= consultasSQL::clean_string($_GET['CodigoSub']);
                 
                $consul=ejecutarSQL::consultar("SELECT * FROM producto WHERE CodigoProd=$CodigoProducto");
        
                    $escomida=mysqli_fetch_array($consul,MYSQLI_ASSOC);
                    if ($escomida['esComida'] == 1) {
                
                        echo '
                                    <script>
                                    window.location = "creaCombo.php?codigoProd='.$escomida['CodigoProd'].'&subCat='.$escomida['subCat'].'";
                                    </script>
                                    ';
                
                
                    } else {
                        $imagen=$escomida['Imagen'];
    $productoinfo=  ejecutarSQL::consultar("SELECT * FROM subproductos WHERE CodigoSub='".$codigoSub."'");
    while ($fila=mysqli_fetch_array($productoinfo, MYSQLI_ASSOC)) {
        echo '
                                                <div class="col-xs-12 col-sm-6">
                                                    <h3 class="text-center">Información de producto</h3>
                                                    <br><br>
                                                    <h4><strong>Nombre: </strong>'.$fila['Modelo'].'</h4><br>
        ...
        if ($imagen['Imagen']!="" && is_file("./assets/img-products/".$imagen['Imagen'])) {
            $imagenFile="./assets/img-products/".$imagen['Imagen'];
        } else {
